...
fn isInvalidId(id: u64) bool {
    ...
    for (2..nrDigits + 1) |i| {
        if (nrDigits % i == 0) {
            const divisor = std.math.pow(u64, 10, nrDigits / i);
            var parts: [100]u64 = undefined;
            var remainder: u64 = id;
            for (0..i) |j| {
                parts[j] = remainder % divisor;
                remainder = (remainder - parts[j]) / divisor;
            }

            const first_part = parts[0];
            var nr_equal: u32 = 0;
            for (1..i) |j| {
                if (parts[j] == first_part) {
                    nr_equal += 1;
                }
            }
            if (nr_equal == i - 1) return true;
        }
    }
