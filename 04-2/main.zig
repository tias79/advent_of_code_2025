...
const Coordinate = struct { x: i64, y: i64 };
...
pub fn main() !void {
    ...
    while (true) {
        var rollsToBeRemoved = std.ArrayList(Coordinate){};
        defer rollsToBeRemoved.deinit(allocator);
        var y: i32 = 0;
            ...
            while (x < matrix.width) : (x += 1) {
                if (matrix.get(x, y) == '@') {
                    ...
                    if (nrAdjacentRolls < 4) {
                        result += 1;
                        rollsToBeRemoved.append(allocator, Coordinate{ .x = x, .y = y }) catch {};
                    }
                }
            }
        }
        for (rollsToBeRemoved.items) |coord| {
            try matrix.set(coord.x, coord.y, '.');
        }
        if (rollsToBeRemoved.items.len == 0) {
            break;
        }
    }
